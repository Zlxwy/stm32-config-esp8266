...
void USART3_IRQHandler(void)
{
    if (USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)
    {
        ...
        if (!is_timer_enabled)    //判断定时器是否已打开
        {
...
void esp8266_uart_init(void)
{
    ...
    USART_InitStructure.USART_WordLength = USART_WordLength_8b;                      //8位数据位，若有校验位则已包含校验位
    ...
    USART_ITConfig(USART3, USART_IT_RXNE, ENABLE);//开启中断接收
...
void esp8266_timer_init(void)
{
    ...
    TIM_TimeBaseInitStructure.TIM_Prescaler = 7200-1;  //100Hz-10ms
